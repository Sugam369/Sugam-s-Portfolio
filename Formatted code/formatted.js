...
function visualmode() {
  ...
  let switchBox = document.getElementById("switchforvisualmode");
  let elements = document.querySelectorAll(".needtobeinvert");

  if (switchBox.checked) {
    body.classList.remove("light-mode");
    body.classList.add("dark-mode");
    elements.forEach((element) => element.classList.add("invertapplied"));
    localStorage.setItem("theme", "dark");
  } else {
    body.classList.remove("dark-mode");
    body.classList.add("light-mode"); // Default mode
    elements.forEach((element) => element.classList.remove("invertapplied"));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  switchBox.checked = savedTheme === "dark";
});
...
console.log('%c Designed and Developed by Sugam Baral ', 'background-image: linear-gradient(90deg,#8000ff,#6bc5f8); color: white;font-weight:900;font-size:1rem; padding:20px;');
